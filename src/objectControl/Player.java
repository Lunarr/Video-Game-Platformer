...
public class Player extends MapObjects			//we're Inheriting everything from MapObject
{
	//player stuff, switch accordingly for ninja
	private int health;
	...
	private final int[] numFrames = 
		{3,4,1,1,4,2,5};				//you can change these depending on how many animations you have for ninja
...
//movement, don't change these anymore, these are just fine
		...
		//load sprites
		try{
			BufferedImage spritesheet = ImageIO.read(getClass().getResourceAsStream("/Sprites/Player/test.png"));	//k lets fetch these sprites
...
////////////////////////////////
//checking attack on Boss
////////////////////////////////
	public void checkAttack(Boss1 boss)
	{
			...
			if (scratching)
			{
				if(facingRight)
				{
					if(boss.getx() > x && boss.getx() < x + scratchRange && boss.gety() > y - height / 2 && boss.gety() < y + height / 2)
					{
						boss.hit(scratchDamage);
					}
			...
			for(int j = 0; j < fireballs.size(); j++)
			{
				if(fireballs.get(j).intersects(boss))
						{
						boss.hit(fireBallDamage);
						fireballs.get(j).setHit();
						break;
						}
			}
			
			//checking enemy hits you
			if(intersects(boss))
			{
				hit (boss.getDamage());
			}
...
//checking attack on enemy
	public void checkAttack(ArrayList<Enemy> enemies)
	{
		
		for(int i = 0; i < enemies.size(); i++)
		{
			Enemy e = enemies.get(i);		//we're going to loop through enemies
			//scratch attack
			if (scratching)
			{
				if(facingRight)
				{		//        my x			
					if(e.getx() > x && e.getx() < x + scratchRange && e.gety() > y - height / 2 && e.gety() < y + height / 2) 		//checking if enemy is in attackrange
					{
						e.hit(scratchDamage);
					}
...
/////////////////////////////////*/
		public void hit (int damage)
		{
			...
			flinching = true;
			flinchTimer = System.nanoTime();				// in update function
		}
	
	
/*////////////////////////////////////////////////
 determines where the player should be after inputs, all my movement
///////////////////////////////////////////////*/
...
/////////////////////////////*/
	public void update()
	{
		...
		if(flinching)
		{
			...
			if(elapsed > 1000)				//1 second of invisibility
			{
				flinching = false;
			}
